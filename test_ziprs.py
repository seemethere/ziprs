...
class TestZiprs(unittest.TestCase):
    ...
    def test_zip_files_and_directories(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                self.assertIn("subdir/subfile.txt", names, "subdir/subfile.txt not found in zip")
    ...
    def test_zip_empty_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                self.assertEqual(len(names), 1, f"Expected 1 entry for empty directory, got {len(names)}: {names}")
                self.assertIn("empty_dir/", names, "The empty directory itself should be in the zip names.")
    ...
    def test_zip_preserves_permissions(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for filename, content, perms, _ in test_cases:
                ...
                self.assertEqual(actual_perms, perms, 
                               f"Failed to set permissions for {filename}")
            ...
            expected_perms = {zip_filename: perms for _, _, perms, zip_filename in test_cases}
            ...
            with zipfile.ZipFile(zip_path, 'r') as zf:
                for info in zf.infolist():
                    ...
                    if filename in expected_perms:
                        ...
                        self.assertEqual(unix_permissions, expected, 
    ...
    def test_unzip_basic(self):
        ...
        with tempfile.TemporaryDirectory() as source_temp_dir, tempfile.TemporaryDirectory() as extracted_temp_dir:
            ...
            os.makedirs(extracted_dir_path, exist_ok=True) # unzip_files should also be able to create this
            ...
            extracted_file1 = os.path.join(extracted_dir_path, os.path.basename(file1_orig_path))
            extracted_subdir = os.path.join(extracted_dir_path, os.path.basename(subdir_orig_path))
            extracted_subfile = os.path.join(extracted_subdir, os.path.basename(subfile_orig_path))
            
            self.assertTrue(os.path.exists(extracted_file1), "file1.txt should be extracted")
            self.assertTrue(os.path.isdir(extracted_subdir), "subdir should be extracted as a directory")
            self.assertTrue(os.path.exists(extracted_subfile), "subdir/subfile.txt should be extracted")
            ...
            self.assertEqual(perms_ext_file1, 0o644,
            ...
            self.assertEqual(perms_ext_subfile, 0o755,
    ...
    def test_unzip_to_non_existent_destination(self):
        ...
        with tempfile.TemporaryDirectory() as source_temp_dir, tempfile.TemporaryDirectory() as extracted_base_dir:
            ...
            ziprs.unzip_files(zip_file_path, extracted_dir_path) # Should not raise error

            # Verify the directory and file were created
            self.assertTrue(os.path.exists(extracted_dir_path), "Destination directory should have been created.")
            self.assertTrue(os.path.isdir(extracted_dir_path), "Destination path should be a directory.")
            
            extracted_file = os.path.join(extracted_dir_path, "dummy.txt")
            self.assertTrue(os.path.exists(extracted_file), "dummy.txt should be extracted into the new directory.")
    ...
    def test_unzip_empty_directory(self):
        ...
        with tempfile.TemporaryDirectory() as source_temp_dir, tempfile.TemporaryDirectory() as extracted_temp_dir:
            ...
            extracted_empty_subdir = os.path.join(extracted_dir_path, os.path.basename(empty_subdir_orig_path))
            self.assertTrue(os.path.exists(extracted_empty_subdir), "Empty subdirectory should be extracted.")
            self.assertTrue(os.path.isdir(extracted_empty_subdir), "Extracted path for empty subdirectory should be a directory.")
            self.assertEqual(len(os.listdir(extracted_empty_subdir)), 0, "Extracted empty subdirectory should be empty.")
